...
def getvalue():
    ...
    if restecg == 'Normal':
        ...
        restecg = 1
    elif restecg == 'Left Ventricular Hyperthrophy':
        restecg = 2
    ...
    if op == 0:
        ...
        opstr = "Error occurred during prediction"
...
def makeWebhookResult(req):
    ...
    if query_response.get("action") == "DefaultWelcomeIntent.DefaultWelcomeIntent-custom":
        ...
        if problem == "Chest Pain":
            soln = "A pain reliever, such as aspirin, can help alleviate the heart/chest pain associated with less severe cases. When heart pain strikes, lying down immediately with the head elevated above the body may bring some relief. A slightly upright position helps when the pain is due to reflux."
        elif problem == "High Blood Pressure":
            soln = "Blood pressure often increases as weight (Obesity) increases. Regular physical activity such as 150 minutes a week can lower your blood pressure by about 5 to 8 mm Hg if you have high blood pressure. Eating a diet that is rich in whole grains, fruits, vegetables and low-fat dairy products and skimps on saturated fat and cholesterol can lower your blood pressure by up to 11 mm Hg if you have high blood pressure. Chronic stress and also smoking may contribute to high blood pressure, so avoid that."
        elif problem in ("breathing problems", "breathlessness"):
            soln = "Breathing-in deeply through the abdomen and also ursed-lip breathing can help to manage your breathlessness. Finding a comfortable and supported position to stand or lie in can help to relax and catch your breath. Inhaling steam can help to keep nasal passages clear, which can help to breathe more easily. Drinking black coffee may help to treat breathlessness, reducing tiredness in the airway muscles. Being overweight also can cause disrupted breathing while you sleep (sleep apnea)."
        elif problem in ("sleeping", "sleep"):
            soln = "Set yourself up for restful sleep: Stick to a regular sleep/wake schedule. Turn off the TV, computer, and other devices before bedtime. Keep your bedroom cool and dark. Avoid alcohol before bedtime and caffeine in the afternoon or evening. Exercise every morning."
    ...
    if query_response.get("action") == "Suffering_Patient_sym2.Suffering_Patient_sym2-custom.Suffering_Patient_sym3-custom":
        ...
        if q1 == 1:
            new_report += "You had Heart Disease before, "
        if q2 == 1:
            new_report += "You have/had Diabetes, "
        if q3 == 1:
            new_report += "You have/had High Blood Pressure, "
        if q4 == 1:
            new_report += "You've suffered from Asthma OR Chronic Obstructive lung disease, "
        if q5 == 1:
            new_report += "You're Smoking (or smoked before), "
        if q6 == 1:
            new_report += "You have/had: Brain Stroke OR Kidney Disease OR Obesity Problem, "
        if sym1 not in ("none", "None") or sym2 not in ("none", "None") or sym3 not in ("none", "None"):
            new_report += "and also have symptoms like "
        if sym1 not in ("none", "None"):
            new_report += sym1 + ", "
        if sym2 not in ("none", "None"):
            new_report += sym2 + ", "
        if sym3 not in ("none", "None"):
            new_report += sym3
        if (q1, q2, q3, q4, q5, q6) == 0:
            new_report += "You did'nt have any of the above mentioned symptoms!"
